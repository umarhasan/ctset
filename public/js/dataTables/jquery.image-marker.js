  ...
  'use strict';
    ...
    $.extend( Plugin.prototype, {
      ...
      addTextbox: function(marker) {
        ...
        var $title = $('<h2 class="image-marker__text-box__title" ><input type="text" class="smal"><input class="lg_lable" type="text"></h2>');
        if (!!marker.title) $title.find('input:eq(1)').val(marker.title);
		if (!!marker.opt) $title.find('input:eq(0)').val(marker.opt);
        $title.on('change keydown paste input', function() {
          marker.title = $title.find('input:eq(1)').val();
		  
		  marker.opt = $title.find('input:eq(0)').val();;
        });
		
        $text_box.append($title);

        /*var $content = $('<p class="image-marker__text-box__content" contenteditable="true"></p>');
