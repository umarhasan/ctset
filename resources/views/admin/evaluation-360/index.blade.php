...
$(document).ready(function(){
    ...
    function loadTable() {
        $.get('evaluation-360', function(res) {
            let html = '';
            ...
            if(role === 'Admin') {
                ...
                // Assessor/Trainee
                res.forEach((r, index) => {
                    r.shares.forEach(share => {
                        html += `<tr>
                            <td>${index + 1}</td>
                            <td>${share.name}</td>
                            <td>${r.title}</td>
                            <td>${share.shared_by_name}</td>
                            <td>${share.assigned_to_name ?? '-'}</td>
                            <td>${share.email ?? '-'}</td>
                            <td>${share.pin ?? '-'}</td>
                            <td>${share.status}</td>
                            <td>${share.status === 'W' ? '<button class="btn btn-success btn-sm">Accept</button>' : '-'}</td>
                            <td><button class="btn btn-info btn-sm" onclick="viewForm(${r.id})">View</button></td>
                        </tr>`;
                    });
                });
            }

            $('#formTable tbody').html(html);
        });
    }
    ...
    window.openShare = function(formId){
    ...
    window.addSection = function(){ $('#sectionsContainer').append($('#sectionTemplate').html()); };
    window.toggleExpand = function(btn){
        let row = $(btn).closest('tr');
        if(row.data('open')){ row.next('.expand-row').remove(); row.data('open', false); $(btn).text('+'); }
        else { row.after($('#expandTemplate').html()); row.data('open', true); $(btn).text('-'); }
    };
    window.removeSection = function(btn){ let row=$(btn).closest('tr'); if(row.data('open')) row.next('.expand-row').remove(); row.remove(); };
    @endrole

    // Assessor/Trainee view form
    window.viewForm = function(id){
        $.get(`evaluation-360/${id}/edit`, res => {
            let html = `<p><strong>Title:</strong> ${res.title}</p><p><strong>Status:</strong> ${res.status}</p>`;
            res.sections.forEach(s => {
