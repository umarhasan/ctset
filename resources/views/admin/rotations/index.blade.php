...
<div class="card card-primary card-outline">
    <div class="card-header d-flex justify-content-between">
        <h3 class="card-title">Rotations</h3>
        <button class="btn btn-primary btn-sm" onclick="openCreateModal()">
            <i class="fa fa-plus"></i> Add Rotation
        </button>
    </div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <button class="btn btn-danger btn-sm" onclick="deleteRecord({{ $rot->id }})"><i class="fa fa-trash"></i></button>
                    </td>
                </tr>
                @endforeach
            </tbody>
...
$(function(){
    ...
    window.openCreateModal = function(){
        $('#record_id').val('');
        $('#masterForm')[0].reset();
        $('#modalTitle').text('Add Rotation');
        new bootstrap.Modal('#masterModal').show();
    }

    window.openEditModal = function(id){
        $.get(`rotations/${id}/edit`, function(res){
            $('#record_id').val(res.id);
            $('#title').val(res.title);
            $('#modalTitle').text('Edit Rotation');
            new bootstrap.Modal('#masterModal').show();
        });
    }
    ...
    $('#masterForm').submit(function(e){
        ...
        let url = id ? `rotations/${id}` : `rotations`;
    ...
    window.deleteRecord = function(id){
        ...
        $.ajax({ url:`rotations/${id}`, method:'DELETE', data:{_token:'{{ csrf_token() }}'}, success:function(){ location.reload(); } });
    }
});
</script>
@endpush
